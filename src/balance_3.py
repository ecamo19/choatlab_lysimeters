...
# Create txt file for storing data from channel 0 -------------------------------
with open(f'../{serial_number}_data/{serial_number}_channel_3_data.txt', 'w') as file:
...
def balance_3_main():
	...
	Log.enable(
		LogLevel.PHIDGET_LOG_INFO, f'../{serial_number}_data/{serial_number}_logs/phidgetlog_3.log'
	)
	...
	try:
		input('Clear any weight over the scale connected to the channel 3 and then press Enter\n')
	except (Exception, KeyboardInterrupt):
		pass

	v1 = voltageRatioInput3.getVoltageRatio()

	#
	try:
		w2 = input(
			'Place a known weight in grams on the scale connected to the channel 3, type the weight in grams, and then press Enter:\n'
		)
	except (Exception, KeyboardInterrupt):
		pass

	# Linear calibration --------------------------------------------------------
	v2 = voltageRatioInput3.getVoltageRatio()
	...
	calibrated = True
	try:
		input('Press Enter to Stop the scale connected to the channel 3\n')
	except (Exception, KeyboardInterrupt):
		pass
