...
def balance_0_main():
	...
	Log.enable(LogLevel.PHIDGET_LOG_INFO, f'../{serial_number}_data/{serial_number}_logs/phidgetlog_0.log')
	...
	try:
		input(
			'Clear any weight over the scale connected to the channel 0 and then press Enter\n'
		)
	...
	try:
		w2 = input(
			'Place a known weight in grams on the scale connected to the channel 0, type the weight in grams, and then press Enter:\n'
		)
	except (Exception, KeyboardInterrupt):
		pass
	...
	calibrated = True
	try:
		input('Press Enter to Stop the scale connected to the channel 0\n')
	except (Exception, KeyboardInterrupt):
		pass
